...
protected:
    ...
    void cleanupTestFiles() {
        const std::vector<std::string> testFiles = {"e2e_test.json",
                                                    "e2e_roundtrip.json",
                                                    "e2e_export.csv",
                                                    "e2e_export.xml",
                                                    "e2e_complex.json",
...
TEST_F(E2EWorkflowTest, AnalysisAndExportWorkflow) {
    ...
    // Export to CSV
    {
        ...
        auto exportResult = analysisService.exportResults(
            analysisResult.value, ExportFormat::CSV, csvFile, truss);
    ...
    // Export to XML
    {
        ...
        auto exportResult = analysisService.exportResults(
            analysisResult.value, ExportFormat::XML, xmlFile, truss);
        EXPECT_TRUE(exportResult.success);
