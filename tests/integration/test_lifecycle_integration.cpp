...
TEST_F(LifecycleIntegrationTest, AddMemberAfterNodesSucceeds) {
    ...
    auto memberResult = trussService->addMember(
        handle,
    ...
    EXPECT_TRUE(memberResult.success) 
        << "Adding member between valid nodes should succeed: " 
...
TEST_F(LifecycleIntegrationTest, AnalyzeWithoutNodesFailsGracefully) {
    ...
    EXPECT_FALSE(validationResult.value.isValid()) 
        << "Empty truss should fail validation";
    EXPECT_GT(validationResult.value.getIssueCount(), 0) 
...
TEST_F(LifecycleIntegrationTest, MemberAdditionWithInvalidNodeIDFails) {
    ...
    EXPECT_FALSE(memberResult.success) 
        << "Adding member with invalid node ID should fail";
    EXPECT_FALSE(memberResult.errorMessage.empty())
...
TEST_F(LifecycleIntegrationTest, MemberAdditionWithNodeIDZeroFails) {
    ...
    EXPECT_FALSE(memberResult.success) 
        << "Node ID 0 is invalid and should be rejected";
}
