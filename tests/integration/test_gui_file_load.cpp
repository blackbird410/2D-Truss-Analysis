...
TEST_F(GUIFileLoadTest, PathConversion_QStringToFilesystem) {
    ...
    qDebug() << "[TEST] Converted std::filesystem::path:" << QString::fromStdString(fsPath.string());
...
TEST_F(GUIFileLoadTest, UIPath_ViaProjectController) {
    ...
    QObject::connect(projectController.get(), &ProjectController::projectOpened,
        [&](TrussHandle handle, const QString& filepath) {
            ...
            qDebug() << "[SIGNAL] projectOpened - handle:" << handle << "path:" << filepath;
        });
    
    QString errorMsg;
    QObject::connect(projectController.get(), &ProjectController::operationFailed,
        [&](const QString& error) {
            errorMsg = error;
