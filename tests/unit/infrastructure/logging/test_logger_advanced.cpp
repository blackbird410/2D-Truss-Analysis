...
protected:
    ...
    void cleanupTestFiles() {
        const std::vector<std::string> testFiles = {"test_concurrent.txt",
                                                    "test_very_long.txt",
                                                    "test_special_chars.txt",
                                                    "test_unicode.txt",
                                                    "test_rapid_fire.txt",
                                                    "test_level_change.txt",
                                                    "test_empty_msg.txt",
                                                    "test_multiline.txt",
                                                    "test_large_file.txt",
                                                    "test_readonly.txt",
                                                    "test_nested_dir/test.txt",
                                                    "stress_test.txt",
                                                    "repeated_test.txt",
                                                    "shared_file.txt",
                                                    "default_test.txt"};
        ...
        for (const auto& file : testFiles) {
            if (std::filesystem::exists(file)) {
                std::filesystem::permissions(
                    file, std::filesystem::perms::owner_write, std::filesystem::perm_options::add);
...
TEST_F(LoggerAdvancedTest, MixedLevelRapidLogging) {
    ...
    for (int i = 0; i < 100; ++i) {
        switch (i % 6) {
            case 0:
                logger->trace("Trace " + std::to_string(i));
                break;
            case 1:
            ...
            case 2:
                ...
                break;
            case 3:
                logger->warn("Warning " + std::to_string(i));
                break;
            case 4:
                logger->error("Error " + std::to_string(i));
                break;
            case 5:
                logger->critical("Critical " + std::to_string(i));
                break;
        }
    }
...
TEST_F(LoggerAdvancedTest, AlternatingLoggersStressTest) {
    ...
    constexpr LogLevel kLevels[] = {LogLevel::Trace,
                                    LogLevel::Debug,
                                    LogLevel::Info,
                                    LogLevel::Warning,
                                    LogLevel::Error,
