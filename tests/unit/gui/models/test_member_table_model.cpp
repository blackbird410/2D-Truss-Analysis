...
public:
    explicit StubTrussView(std::vector<MemberView> members = {})
...
TEST_F(MemberTableModelTest, HorizontalHeaderLabels)
{
    const std::vector<std::string> expected = {
        "ID", "Start", "End", "Material", "E [GPa]", "A [cm²]",
        "Length [m]", "Angle [°]", "Force [kN]", "Stress [MPa]", "Util. Ratio", "State",
    };
...
TEST_F(MemberTableModelTest, ResultsColumnsShowDashBeforeResults)
{
    using M = truss::gui::model::MemberTableModel;
    StubTrussView view(
        {makeMember(1, 200e9, 1e-4, 1.0, 0.0, 50000.0, 500e6, 0.8, true, false)});
    model.refresh(view);

    for (int col : {M::kColForce, M::kColStress, M::kColRatio, M::kColState}) {
        EXPECT_EQ(model.data(model.index(0, col)).toString(), QStringLiteral("—"))
            << "Col " << col;
    }
}
...
TEST_F(MemberTableModelTest, StateString_Yielded)
{
    using M = truss::gui::model::MemberTableModel;
    StubTrussView view(
        {makeMember(1, 200e9, 1e-4, 1.0, 0.0, 0, 0, 1.2, true, /*yielded=*/true)});
    model.refresh(view);
    model.setHasResults(true);
...
TEST_F(MemberTableModelTest, StateString_Compression)
{
    using M = truss::gui::model::MemberTableModel;
    StubTrussView view(
        {makeMember(1, 200e9, 1e-4, 1.0, 0.0, 0, 0, 0.5, /*inTension=*/false)});
    model.refresh(view);
    model.setHasResults(true);
