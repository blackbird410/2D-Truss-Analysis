...
TEST_F(TrussEditControllerAdvancedTest, OperationWithoutValidHandle) {
    // Don't set handle
    auto invalidController = std::make_unique<TrussEditController>(mockService.get(), *presenter);
...
TEST_F(TrussEditControllerAdvancedTest, AllSupportTypeChanges) {
    const std::vector<core::SupportType> supportTypes = {core::SupportType::Free,
