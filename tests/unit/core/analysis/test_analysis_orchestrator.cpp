...
protected:
    ...
    Truss createSimpleTruss() {
        ...
        auto member3 = std::make_shared<Member>(
            3, node1, node3, material, section);  // Bottom chord
     ...
     */
    void compareResults(const std::vector<Real>& expected,
        ...
        for (size_t i = 0; i < expected.size(); ++i) {
            EXPECT_NEAR(expected[i], actual[i], tolerance) << context << " at index " << i;
        }
    }
};
...
TEST_F(AnalysisOrchestratorTest, WarrenTruss_ComplexStructure) {
    ...
    for (size_t i = 0; i < results.memberForces.size(); ++i) {
        EXPECT_TRUE(std::isfinite(results.memberForces[i])) << "Non-finite force in member " << i;
    }
}
...
TEST_F(AnalysisOrchestratorTest, StiffnessMatrixAssembly) {
    ...
    for (size_t i = 0; i < results.stiffnessMatrix.size(); ++i) {
        for (size_t j = 0; j < results.stiffnessMatrix[i].size(); ++j) {
            EXPECT_NEAR(results.stiffnessMatrix[i][j], results.stiffnessMatrix[j][i], 1e-10)
...
TEST_F(AnalysisOrchestratorTest, ValidationRejectsTruss_FatalErrors) {
    ...
    EXPECT_THROW(
        { orchestrator.analyze(emptyTruss); }, std::runtime_error)
        << "Empty truss should throw due to validation failure";
}
...
TEST_F(AnalysisOrchestratorTest, ValidationRejectsTruss_Errors) {
    ...
    EXPECT_THROW(
        { orchestrator.analyze(truss); }, std::runtime_error)
        << "Truss with zero-length members should throw due to validation failure";
}
