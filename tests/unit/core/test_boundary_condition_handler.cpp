...
protected:
    void SetUp() override { handler = std::make_unique<BoundaryConditionHandler>(); }
...
TEST_F(BoundaryConditionHandlerTest, ApplyToStiffness_ReducedMatrix) {
    ...
    MatrixXd K(6, 6);
    K << 10, 1, 2, 3, 4, 5, 1, 20, 6, 7, 8, 9, 2, 6, 30, 10, 11, 12, 3, 7, 10, 40, 13, 14, 4, 8, 11,
        13, 50, 15, 5, 9, 12, 14, 15, 60;
...
TEST_F(BoundaryConditionHandlerTest, MixedSupportTypes) {
    ...
    // Verify free DOF values placed correctly
    for (size_t i = 0; i < freeDofs.size(); ++i) {
        EXPECT_NEAR(U(freeDofs[i]), Uf(i), 1e-10) << "Mismatch at free DOF index " << freeDofs[i];
    }

    // Verify constrained DOFs are zero
    for (Index dof : constrainedDofs) {
        EXPECT_NEAR(U(dof), 0.0, 1e-10) << "Constrained DOF " << dof << " should be zero";
    }
}
