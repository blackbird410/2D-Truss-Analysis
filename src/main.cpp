    ...
    try {
        ...
        if (!dbManager.initialize()) {
            ...
            QMessageBox::critical(
                nullptr, "Database Error", "Failed to initialize database system");
            return false;
        }
        ...
        if (!coreApp.initialize()) {
            ...
            QMessageBox::critical(
                nullptr, "Application Error", "Failed to initialize core application");
            return false;
        }
...
int main(int argc, char* argv[]) {
    ...
    splash.show();
    splash.showMessage(
        "Loading 2D Truss Analysis...", Qt::AlignCenter | Qt::AlignBottom, Qt::white);

    app.processEvents();

    // Initialize application components
    splash.showMessage("Initializing components...", Qt::AlignCenter | Qt::AlignBottom, Qt::white);
    app.processEvents();
    ...
    try {
        ...
        QTimer::singleShot(1000, [&splash, &mainWindow]() { splash.finish(mainWindow.get()); });
        ...
        QMessageBox::critical(
            nullptr, "Application Error", QString("Failed to start application: %1").arg(e.what()));
        return -1;
    }
}
