...
public:
    ...
    MemberPtr addMember(const NodePtr& startNode,
    ...
    // Find methods
    NodePtr findNearestNode(const Point2D& point,
                            Real maxDistance = std::numeric_limits<Real>::max()) const;
    MemberPtr findNearestMember(const Point2D& point,
                                Real maxDistance = std::numeric_limits<Real>::max()) const;
