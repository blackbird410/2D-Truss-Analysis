...
std::vector<ValidationIssue> ValidationResult::getIssuesByCategory(const std::string& category) const {
    std::vector<ValidationIssue> filtered;
    for (const auto& issue : m_issues) {
...
std::string ValidationResult::getSummary() const {
    ...
    size_t errors = countBySeverity(ValidationSeverity::Error) + countBySeverity(ValidationSeverity::Fatal);
...
void TrussValidator::validateStructuralCompleteness(const Truss& truss, ValidationResult& result) const {
...
void TrussValidator::validateGeometry(const Truss& truss, ValidationResult& result) const {
    ...
    for (auto memberId : zeroLengthMembers) {
        ...
        if (member) {
            ValidationIssue issue(
                ...
                "Geometry",
                "Member " + std::to_string(memberId) + " has zero or near-zero length",
                "Member length: " + std::to_string(member->getLength()) + " m. "
                "Zero-length members cannot carry loads and will cause singular stiffness matrix."
            );
    ...
    for (const auto& [id1, id2] : coincidentPairs) {
        ValidationIssue issue(
            ...
            "Geometry",
            "Nodes " + std::to_string(id1) + " and " + std::to_string(id2) + " are coincident or very close",
    ...
    for (const auto& [id1, id2] : duplicatePairs) {
        ValidationIssue issue(
            ValidationSeverity::Warning,
            "Geometry",
            "Members " + std::to_string(id1) + " and " + std::to_string(id2) + " connect the same nodes",
    ...
    for (const auto& node : truss.getNodes()) {
        if (std::isnan(node->getX()) || std::isnan(node->getY()) ||
            std::isinf(node->getX()) || std::isinf(node->getY())) {
            ValidationIssue issue(
                ValidationSeverity::Fatal,
                "Geometry",
                "Node " + std::to_string(node->getId()) + " has invalid coordinates (NaN or infinity)",
                "Position: (" + std::to_string(node->getX()) + ", " + std::to_string(node->getY()) + ")"
            );
    ...
    for (const auto& member : truss.getMembers()) {
        ...
        if (material.youngModulus <= 0.0) {
            ValidationIssue issue(
                ValidationSeverity::Error,
                "Material",
                "Member " + std::to_string(member->getId()) + " has non-positive Young's modulus",
                "Young's modulus: " + std::to_string(material.youngModulus) + " Pa. Must be positive."
            );
        ...
        if (section.area <= 0.0) {
            ValidationIssue issue(
                ValidationSeverity::Error,
                "Material",
                "Member " + std::to_string(member->getId()) + " has non-positive cross-sectional area",
                "Area: " + std::to_string(section.area) + " m². Must be positive."
            );
            issue.affectedMembers.push_back(member->getId());
            result.addIssue(issue);
        }
        ...
        if (material.density <= 0.0) {
            ValidationIssue issue(
                ValidationSeverity::Warning,
                "Material",
                "Member " + std::to_string(member->getId()) + " has non-positive density",
                "Density: " + std::to_string(material.density) + " kg/m³. Self-weight calculations will be incorrect."
            );
            issue.affectedMembers.push_back(member->getId());
            result.addIssue(issue);
        }
        ...
        if (material.yieldStrength <= 0.0) {
            ValidationIssue issue(
                ValidationSeverity::Warning,
                "Material",
                "Member " + std::to_string(member->getId()) + " has non-positive yield strength",
                "Yield strength: " + std::to_string(material.yieldStrength) + " Pa. Stress checking will be invalid."
            );
            issue.affectedMembers.push_back(member->getId());
            result.addIssue(issue);
        }
...
void TrussValidator::validateBoundaryConditions(const Truss& truss, ValidationResult& result) const {
    ...
    for (const auto& node : truss.getNodes()) {
        ...
        if (node->isRoller()) rollerCount++;
    }
    
    result.addIssue(ValidationIssue(
        ValidationSeverity::Info,
        "Boundary Conditions",
        "Support configuration: " + std::to_string(pinnedCount) + " pinned, " + std::to_string(rollerCount) + " roller supports",
...
void TrussValidator::validateStaticDeterminacy(const Truss& truss, ValidationResult& result) const {
    ...
    if (determinacyCheck == 0) {
        result.addIssue(ValidationIssue(
            ...
            "2n = m + r: 2×" + std::to_string(n) + " = " + std::to_string(m) + " + " + std::to_string(r)
        ));
        ...
        result.addIssue(ValidationIssue(
            ValidationSeverity::Error,
            "Static Determinacy",
            "Truss is unstable or has internal mechanisms (deficiency: " + std::to_string(determinacyCheck) + ")",
            "2n = " + std::to_string(2*n) + ", m + r = " + std::to_string(m + r) + ". "
    ...
    for (const auto& node : truss.getNodes()) {
        ...
        if (connectedMembers.empty() && !node->isConstrained()) {
            ValidationIssue issue(
                ValidationSeverity::Warning,
                "Kinematic Stability",
                "Node " + std::to_string(node->getId()) + " is not connected to any member and has no support",
...
void TrussValidator::validateLoads(const Truss& truss, ValidationResult& result) const {
    ...
    for (const auto& node : truss.getNodes()) {
        if (node->hasAppliedForce()) {
            ...
            if (std::isnan(force.fx) || std::isnan(force.fy) ||
                std::isinf(force.fx) || std::isinf(force.fy)) {
                ValidationIssue issue(
                    ValidationSeverity::Error,
                    "Loads",
                    "Node " + std::to_string(node->getId()) + " has invalid force values (NaN or infinity)",
                    "Force: (" + std::to_string(force.fx) + ", " + std::to_string(force.fy) + ")"
                );
                issue.affectedNodes.push_back(node->getId());
                result.addIssue(issue);
            }
            ...
            if (node->getDegreesOfFreedom() == 0) {
                ValidationIssue issue(
                    ValidationSeverity::Warning,
                    "Loads",
                    "Force applied to fully constrained node " + std::to_string(node->getId()),
                    "This force will be entirely resisted by support reactions and not transferred through members."
                );
                issue.affectedNodes.push_back(node->getId());
                result.addIssue(issue);
            }
    ...
    for (const auto& member : truss.getMembers()) {
        ...
        if (!startNode || !endNode) {
            ValidationIssue issue(
                ValidationSeverity::Fatal,
                "Connectivity",
                "Member " + std::to_string(member->getId()) + " has null node connection(s)",
...
std::vector<std::pair<NodeId, NodeId>> TrussValidator::findCoincidentNodePairs(
    const Truss& truss, Real tolerance) const {
...
std::vector<MemberId> TrussValidator::findZeroLengthMembers(
    const Truss& truss, Real tolerance) const {
...
std::vector<std::pair<MemberId, MemberId>> TrussValidator::findDuplicateMembers(
