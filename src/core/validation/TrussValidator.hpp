...
struct ValidationIssue {
    ...
    ValidationIssue(ValidationSeverity sev, const std::string& cat, const std::string& msg, const std::string& detail = "")
...
private:
    ...
    bool hasCoincidentNodes(const Truss& truss, Real tolerance = Constants::GEOMETRY_TOLERANCE) const;
    bool hasDuplicateMembers(const Truss& truss) const;
    bool hasZeroLengthMembers(const Truss& truss, Real tolerance = Constants::GEOMETRY_TOLERANCE) const;
    
    std::vector<std::pair<NodeId, NodeId>> findCoincidentNodePairs(const Truss& truss, Real tolerance) const;
