...
AnalysisResultsPresenter::DisplayData 
AnalysisResultsPresenter::formatResults(
    ...
    const truss::core::interfaces::ITrussView& truss) const 
{
    ...
    // Summary message
    data.summaryMessage = generateSuccessMessage(
        truss.getNodeCount(),
    ...
    for (size_t i = 0; i < nodeViews.size(); ++i) {
        ...
        QString nodeText = QString("Node %1: %2")
            .arg(nodeView.id)
            .arg(formatVector2D(
                displacements[dofIndex] * METERS_TO_MM,
    ...
    for (size_t i = 0; i < memberViews.size(); ++i) {
        ...
        QString forceText = QString("Member %1: %2")
            .arg(memberView.id)
            .arg(formatForce(forces[i]));
        
        QString stressText = QString("Member %1: %2")
            .arg(memberView.id)
            .arg(formatStress(stresses[i]));
...
QString AnalysisResultsPresenter::generateSuccessMessage(
    size_t nodeCount,
    size_t memberCount,
