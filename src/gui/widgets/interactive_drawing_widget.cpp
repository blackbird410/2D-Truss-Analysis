...
DrawingCanvas::DrawingCanvas(application::TrussApplicationService& trussService, QWidget* parent)
    : QWidget(parent), m_trussService(trussService), m_drawingMode(DrawingMode::Select),
      ...
      m_currentMaterial("Steel", 200e9, 7850, 250e6), m_currentSection("Default", 0.002),
      m_isDragging(false), m_isPanning(false), m_currentMouseWorld(0.0, 0.0), m_memberStartNode(0),
...
void DrawingCanvas::drawNodes(QPainter& painter) {
    ...
    for (size_t i = 0; i < nodeViews.size(); ++i) {
        ...
        // Check if selected
        bool isSelected = std::find(m_selectedNodes.begin(), m_selectedNodes.end(), nodeView.id) !=
                          m_selectedNodes.end();
        ...
        // Draw node circle
        painter.drawEllipse(
            screenPos - QPoint(NODE_RADIUS, NODE_RADIUS), 2 * NODE_RADIUS, 2 * NODE_RADIUS);
...
void DrawingCanvas::drawMembers(QPainter& painter) {
    ...
    for (size_t i = 0; i < memberViews.size(); ++i) {
        ...
        auto startNodeIt = std::find_if(nodeViews.begin(),
                                        nodeViews.end(),
                                        [&memberView](const truss::core::interfaces::NodeView& nv) {
                                            return nv.id == memberView.startNodeId;
                                        });
        auto endNodeIt = std::find_if(nodeViews.begin(),
                                      nodeViews.end(),
                                      [&memberView](const truss::core::interfaces::NodeView& nv) {
                                          return nv.id == memberView.endNodeId;
                                      });

        if (startNodeIt == nodeViews.end() || endNodeIt == nodeViews.end())
            continue;
        ...
        // Check if selected
        bool isSelected = std::find(m_selectedMembers.begin(), m_selectedMembers.end(), i) !=
                          m_selectedMembers.end();
...
void DrawingCanvas::drawLoads(QPainter& painter) {
    ...
    for (const auto& nodeView : nodeViews) {
        ...
        QPoint forceEnd(
            ...
            nodePos.y() - static_cast<int>(length * nodeView.fy /
                                           magnitude)  // Inverted Y for screen coordinates
        );
        ...
        double angle = std::atan2(nodeView.fy,
                                  nodeView.fx);  // Correct angle calculation for screen coordinates
        int arrowSize = 8;

        arrowHead << forceEnd
                  << forceEnd + QPoint(static_cast<int>(-arrowSize * std::cos(angle - M_PI / 6)),
        ...
        painter.setFont(m_labelFont);
        painter.drawText(forceEnd + QPoint(5, 5),
                         QString("%1 kN").arg(magnitude / 1000.0, 0, 'f', 1));
    }
}
...
void DrawingCanvas::drawCurrentOperation(QPainter& painter) {
    if (m_drawingMode == DrawingMode::AddMember && m_isCreatingMember && m_memberStartNode > 0) {
        ...
        if (m_trussHandle != 0) {
            ...
            auto it = std::find_if(nodeViews.begin(),
                                   nodeViews.end(),
...
void DrawingCanvas::mousePressEvent(QMouseEvent* event) {
    ...
    switch (m_drawingMode) {
        ...
        case DrawingMode::AddNode: {
            truss::core::Point2D position = m_snapToGrid ? snapToGrid(m_currentMouseWorld)
                                                         : m_currentMouseWorld;
        ...
        case DrawingMode::SetSupport: {
            ...
            if (nodeId > 0 && m_trussHandle != 0) {
                ...
                auto nodeIt = std::find_if(nodeViews.begin(),
                                           nodeViews.end(),
...
bool DrawingCanvas::isNearLine(const QPoint& point,
...
void DrawingCanvas::addMemberBetweenNodes(size_t startNodeId, size_t endNodeId) {
    ...
    truss::application::MaterialSpec material{m_currentMaterial.youngModulus, "Steel"};
    ...
    emit memberAddRequested(static_cast<truss::core::NodeId>(startNodeId),
                            static_cast<truss::core::NodeId>(endNodeId),
                            material,
                            section);
    emit statusMessage(
        QString("Member add requested between nodes %1 and %2").arg(startNodeId).arg(endNodeId));
}
...
void DrawingCanvas::applyLoadToNode(size_t nodeId, const truss::core::Force2D& force) {
    ...
    emit loadApplyRequested(static_cast<truss::core::NodeId>(nodeId), force);
...
void DrawingCanvas::setSupportType(size_t nodeId, truss::core::SupportType supportType) {
    ...
    emit supportSetRequested(static_cast<truss::core::NodeId>(nodeId), supportType);
    ...
    switch (supportType) {
        ...
        case truss::core::SupportType::Pinned:
            ...
            break;
        case truss::core::SupportType::RollerX:
            supportName = "Roller X";
            break;
        case truss::core::SupportType::RollerY:
            supportName = "Roller Y";
            break;
        default:
            supportName = "Unknown";
            break;
    ...
    emit statusMessage(
        QString("Support type '%1' requested for node %2").arg(supportName).arg(nodeId));
}
...
PropertyPanel::PropertyPanel(QWidget* parent) : QWidget(parent), m_canvas(nullptr) {
    setupUI();
...
void PropertyPanel::setDrawingCanvas(DrawingCanvas* canvas) {
    ...
    if (m_canvas) {
        ...
        connect(
            m_canvas, &DrawingCanvas::memberSelected, this, &PropertyPanel::updateFromSelection);
...
void PropertyPanel::setupUI() {
    ...
    connect(m_materialCombo,
            ...
            this,
            &PropertyPanel::onMaterialChanged);
    connect(m_sectionCombo,
            QOverload<int>::of(&QComboBox::currentIndexChanged),
            this,
            &PropertyPanel::onSectionChanged);
    connect(m_nodeXSpin,
            QOverload<double>::of(&QDoubleSpinBox::valueChanged),
            this,
            &PropertyPanel::onNodePositionChanged);
    connect(m_nodeYSpin,
            QOverload<double>::of(&QDoubleSpinBox::valueChanged),
            this,
            &PropertyPanel::onNodePositionChanged);
    connect(m_supportCombo,
            QOverload<int>::of(&QComboBox::currentIndexChanged),
            this,
            &PropertyPanel::onSupportChanged);
    connect(m_forceXSpin,
            QOverload<double>::of(&QDoubleSpinBox::valueChanged),
            this,
            &PropertyPanel::onLoadChanged);
    connect(m_forceYSpin,
            QOverload<double>::of(&QDoubleSpinBox::valueChanged),
            this,
            &PropertyPanel::onLoadChanged);
}
...
void PropertyPanel::onMaterialChanged() {
    ...
    if (index >= 0 && index < static_cast<int>(m_materialPresets.size())) {
        ...
        if (m_canvas) {
            MaterialPreset newMaterial(material.name,
...
void PropertyPanel::onSectionChanged() {
    ...
    if (index >= 0 && index < static_cast<int>(m_sectionPresets.size())) {
        ...
        if (m_canvas) {
            SectionPreset newSection(section.name,
                                     m_areaSpin->value() / 10000);  // Convert back to m²
    ...
    if (selectedNodes.size() == 1) {
        ...
        if (node) {
            ...
            switch (m_supportCombo->currentIndex()) {
                case 0:
                    supportType = truss::core::SupportType::Free;
                    break;
                case 1:
                    supportType = truss::core::SupportType::Pinned;
                    break;
                case 2:
                    supportType = truss::core::SupportType::RollerX;
                    break;
                case 3:
                    supportType = truss::core::SupportType::RollerY;
                    break;
                default:
                    supportType = truss::core::SupportType::Free;
                    break;
...
void InteractiveDrawingWidget::connectSignals() {
    ...
    connect(
        m_selectAction, &QAction::triggered, this, &InteractiveDrawingWidget::onDrawingModeChanged);
    connect(m_addNodeAction,
            &QAction::triggered,
            this,
            &InteractiveDrawingWidget::onDrawingModeChanged);
    connect(m_addMemberAction,
            &QAction::triggered,
            this,
            &InteractiveDrawingWidget::onDrawingModeChanged);
    connect(m_addLoadAction,
            &QAction::triggered,
            this,
            &InteractiveDrawingWidget::onDrawingModeChanged);
    connect(m_setSupportAction,
            &QAction::triggered,
            this,
            &InteractiveDrawingWidget::onDrawingModeChanged);
    ...
    connect(m_gridSpacingSpin,
            QOverload<double>::of(&QDoubleSpinBox::valueChanged),
            m_canvas,
            &DrawingCanvas::setGridSpacing);
    ...
    connect(
        m_canvas, &DrawingCanvas::trussModified, this, &InteractiveDrawingWidget::trussModified);
    connect(
        m_canvas, &DrawingCanvas::statusMessage, this, &InteractiveDrawingWidget::statusMessage);
}
