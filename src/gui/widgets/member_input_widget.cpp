...
MemberInputWidget::MemberInputWidget(truss::application::ITrussService& trussService,
    ...
    : QWidget(parent), m_trussService(trussService), m_editController(editController),
      m_currentTrussHandle(0), m_startNodeCombo(new QComboBox(this)),
      m_endNodeCombo(new QComboBox(this)), m_areaEdit(new QLineEdit(this)),
      m_youngModulusEdit(new QLineEdit(this)), m_densityEdit(new QLineEdit(this)),
      m_yieldStrengthEdit(new QLineEdit(this)), m_addButton(new QPushButton("Add Member", this)),
    ...
    connect(&m_editController,
            &truss_controllers::TrussEditController::memberAdded,
            this,
            &MemberInputWidget::onMemberAdded);
    connect(&m_editController,
            &truss_controllers::TrussEditController::operationFailed,
            this,
            &MemberInputWidget::onOperationFailed);
}
...
void MemberInputWidget::onOperationFailed(const QString& errorMessage) {
    QMessageBox::critical(this, "Error", QString("Failed to add member: %1").arg(errorMessage));
}
