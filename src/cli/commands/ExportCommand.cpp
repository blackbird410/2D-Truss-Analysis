...
int ExportCommand::execute() {
    ...
    if (!validateInputFile(m_resultsFile)) {
        m_presenter.displayError("Results file does not exist or is not readable: " + m_resultsFile);
    ...
    if (m_verbose) {
        ...
        switch(format) {
            case ExportFormat::JSON: formatName = "JSON"; break;
            case ExportFormat::XML: formatName = "XML"; break;
            case ExportFormat::CSV: formatName = "CSV"; break;
            case ExportFormat::TSV: formatName = "TSV"; break;
            case ExportFormat::TXT: formatName = "TXT"; break;
            case ExportFormat::LaTeX: formatName = "LaTeX"; break;
            case ExportFormat::HTML: formatName = "HTML"; break;
        }
    ...
    if (!exportResult) {
        ...
        m_presenter.displayInfo("Suggestion: Check that the output directory exists and is writable.\n");
        m_analysisService.clearResults(resultsHandle);
        m_trussService.clearTruss(trussHandle);
        return 1;
    }
...
ExportCommand::parseExportFormat(const std::string& formatStr) const {
    ...
    if (upper == "TSV") return ExportFormat::TSV;
    if (upper == "TXT") return ExportFormat::TXT;
    if (upper == "LATEX" || upper == "TEX") return ExportFormat::LaTeX;
    if (upper == "HTML") return ExportFormat::HTML;
...
ExportCommand::getDefaultExportFormat(const std::string& filepath) const {
    ...
    if (ext == ".tsv") return ExportFormat::TSV;
    if (ext == ".txt") return ExportFormat::TXT;
    if (ext == ".tex") return ExportFormat::LaTeX;
    if (ext == ".html") return ExportFormat::HTML;
