...
int ValidateCommand::execute() {
    ...
    if (!validateResult) {
        ...
        if (errorMsg.find("support") != std::string::npos) {
            ...
            m_presenter.displayInfo("  - Check that supports are adequate to prevent rigid body motion");
    ...
    m_presenter.displayInfo("Use 'TrussAnalysisCLI analyze --file " + m_inputFile + "' to run analysis.\n");
