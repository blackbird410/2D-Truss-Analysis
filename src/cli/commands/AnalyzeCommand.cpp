...
int AnalyzeCommand::execute() {
    ...
    if (!loadResult) {
        ...
        m_presenter.displayInfo("Suggestion: Verify the file format is valid (JSON, XML, CSV, etc.).\n");
        return 1;
    }
    ...
    if (!validateResult) {
        ...
        m_presenter.displayInfo("Suggestion: Check truss geometry, supports, and loading conditions.\n");
    ...
    if (!analysisResult) {
        ...
        m_presenter.displayInfo("Suggestion: Check that the truss is statically determinate and properly constrained.\n");
        m_trussService.clearTruss(trussHandle);
        return 1;
    }
