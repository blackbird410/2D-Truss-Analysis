...
Result<ResultsHandle> AnalysisApplicationService::analyze(
    ...
    const core::analysis::AnalysisOptions& options)
{
    try {
        ...
        return Result<ResultsHandle>::Failure(
            std::string("Analysis failed: ") + e.what());
    }
}

const core::interfaces::IAnalysisResultsView& AnalysisApplicationService::getResultsView(
    ResultsHandle handle) const
{
...
Result<bool> AnalysisApplicationService::exportResults(
