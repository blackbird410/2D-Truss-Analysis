...
echo "  Building 2D Truss Analysis..."
...
if command -v macdeployqt &> /dev/null; then
    ...
    if [ ! -d "TrussAnalysisGUI.app/Contents/Frameworks/QtDBus.framework" ]; then
        echo "  QtDBus framework missing, adding manually..."
        ...
        if [ -d "$QT_PATH/QtDBus.framework" ]; then
            ...
            echo "  QtDBus framework not found in Qt installation"
        fi
    fi
    ...
    echo "  macdeployqt not found - the app may not work on other systems"
fi
...
if ./TrussAnalysisGUI.app/Contents/MacOS/TrussAnalysisGUI --version &> /dev/null; then
    ...
    echo "  Application test failed or version check not supported"
fi
